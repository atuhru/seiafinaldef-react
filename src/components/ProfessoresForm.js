...
const ProfessoresForm = ({ professores, onSave }) => {
  const [formData, setFormData] = useState({ id: '', nome: '', funcao: '' });

  useEffect(() => {
    if (professores) setFormData(professores);
  }, [professores]);
